...
void bgfg_vibe::init_model(cv::Mat& firstSample)
{
	...
	model->foreGround=new cv::Mat(cv::Size(firstSample.cols,firstSample.rows), CV_8UC1);
	...
	{       
		...
		for(int i=0;i<channels[s].rows;i++)
		{
			...
			for(int j=0;j<channels[0].cols;j++)
			{
				...
				if(s==0)(model->foreGround->data + ioff)[j]=0;
			}
		}
...
cv::Mat* bgfg_vibe::fg(cv::Mat& frame)
{
	...
	for(size_t i=0;i<channels.size();i++)
	{
		...
		if(i>0 && i<2)
		{
			bitwise_or(*model->fgch[i-1],*model->fgch[i],*model->foreGround);
		}
		if(i>=2)
		{
			bitwise_or(*model->foreGround,*model->fgch[i],*model->foreGround);
		}
	}
